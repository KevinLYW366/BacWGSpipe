...
### 2.1 Data Overview of Short-read NGS Data

#### 2.1.1 Raw Data Preprocessing

Preprecessing and quality control are performed on NGS raw data using fastp<sup>[1]</sup> to ensure the accuracy and reliability of downstrea analysi.
...
<center>**Table 2-1 Statistics table of NGS data**</center>
...
Based on the short-read NGS clean data after quality control, Unicycler<sup>[3]</sup> was used to assemble the genome to obtain the sequence file that can reflect the basic situation of the sample genome, and the assembly results were evaluated. The assembly results are shown in the following table:

<center>**Table2-3 Statistics table of genome assembly**</center>
...
Prokka<sup>[4]</sup> is used to predict coding genes from genome assembly. Statistics of coding genes prediction results:
...
Prokka<sup>[4]</sup> is used to predict tRNA and rRNA. Statistics of ncRNAs prediction results:
...
The PlasmidFinder database<sup>[5]</sup> is a plasmid replication subdatabase based on manual proofreading. Plasmid sequences were identified from bacterial genome sequencing data using Blast, which compared the annotation results of the assembled genome with the PlasmidFinder database. Meanwhile, Platon<sup>[6]</sup> was used to detect plasmid sequence. Platon has a specially developed algorithm flow for plasmid sequence detection.
...
<center>**Table 2-7 Summary statistics table of plasmid prediction from Platon**</center>

```{r echo=FALSE, message=FALSE, warning=FALSE}
platon_results_df <- read.table(
  params$platon_results, 
  header = T,
  sep = "\t",
  comment.char = "", 
  check.names = FALSE
...
sample_list <- scan(params$pass_sample_list, character())
for (s in sample_list) {
  if (! s %in% platon_results_df$sample) {
    platon_results_df[nrow(platon_results_df) + 1,] = list(s, 0, 0, 0)
  }
}
...
datatable(
  ...
  class = "hover cell-border order-column stripe",
  rownames = FALSE,
  colnames = c("Sample ID", "Platon number (#)", "Total length (bp)",
               "Average length (bp)"),
  extensions = c("Scroller", "FixedColumns", "Buttons"),
  option = list(
    scrollX = TRUE,
...
- Directory of result files: results/03.genome_component/*/platon;
- Summary result file: results/03.genome_component/all_platon_results.xls.
...
Phigaro<sup>[7]</sup> was used to predict the prophage on the genome assembly. Phigaro annotates the phage gene using the hidden Markov model (HMM) of the pVOGs (pVOGs) spectra of prokaryotes and viruses.

<center>**Table 2-8 Summary statistics table of Prophage prediction results**</center>
...
ICEfinder<sup>[8]</sup> can quickly detect ICE for bacterial genome sequence, and the ICE types detected include T4SS-type ICEs, AICEs and IMEs.

<center>**Table 2-9 Summary statistics table of ICE prediction results**</center>
...
digIS<sup>[9]</sup> is based on the pHMM model assembled by the catalytic domain of transposition enzyme, and shows very good performance in detecting known IS. At the same time, digIS also has certain detection ability for detecting remote IS and speculated new IS.

<center>**Table 2-10 Summary statistics table of IS prediction results**</center>
...
Based on phylogenetic composition, IslandPath-DIOMB<sup>[10]</sup> is applied to predict gene islands. IslandPath-DIOMB identifies gene islands and potential horizontal gene transfer by detecting dinonotide bias and mobility genes in the phylogenetic system, such as transposase or integrase.

<center>**Table 2-11 Summary statistics table of GI prediction results**</center>
...
1) The protein sequences of predicted genes were mapped to eggNOG database<sup>[13]</sup> using eggNOG-mapper<sup>[12]</sup>;
...
Therefore, secreted proteins were analyzed by signal peptide prediction and transmembrane helix structure prediction. The signal peptide prediction tool signalP<sup>[18]</sup> and the transmembrane helix structure prediction tool TMHMM<sup>[19]</sup> were used to predict secreted proteins. Protein coding genes with signal peptide but without transmembrane helical structure outside the signal peptide region were selected as candidate secreted protein coding genes.

<center>**Table 2-13 Statistics table of secreted protein prediction results**</center>
...
VFDB database is called Virulence Factors of Pathogenic Bacteria<sup>[20]</sup>. It's used for the study of pathogenic bacteria, chlamydia and mycoplasma. In addition to the species information and description of basic characteristics of virulence genes, It also provides a detailed description of virulence gene functions and pathogenic mechanisms.

Blast<sup>[21]</sup> was used to compare the nucleic acid sequence of the coding gene of the target species with the VFDB database, and the gene of the target species and its corresponding functional annotation information of virulence factors were combined to obtain annotation results.
...
CARD Database is called Comprehensive Antibiotic Research Database<sup>[22]</sup>, this database is a newly emerged resistance gene database in recent years, it has the advantages of comprehensive information, user-friendly, timely update and maintenance. The core composition of the database is Antibiotic Resistance Ontology (ARO), which integrates the sequence, antibiotic resistance, action mechanism, correlation between ARO and other information. Through the annotation of the database, we can find the name of antibiotic resistance related genes, the antibiotic type to be tolerated and so on.

Resistance Gene Identifier(RGI<sup>[23]</sup>) was used to map amino acid sequences of target species to CARD database (RGI built-in blastp, default evalue ≤ 1e-30). According to the comparison results of RGI, the resistance gene information annotated to the database was counted.
...
MLST is performed by a tool called mlst<sup>[25]</sup> based on PubMLST database<sup>[26]</sup>. (**Strains without typing scheme in PubMLST database are not available for MLST typing**)

<center>**Table 2-14 Summary statistics table of MLST results**</center>
...
Comparative genomics analysis of bacterial genomes is a process of studying the evolutionary relationships among different bacterial species or strains. To identify the phylogeny of the bacterial genomes, pan-genome analysis is performed by Roary<sup>[27]</sup> to generate the core genome alignment, which will then be used by BacWGSpipe to construct a maximum-likelihood phylogenetic tree with IQ-TREE<sup>[28]</sup>. The output tree can be viewed and annotated by any tree-viewing software such as ITOL<sup>[29]</sup>. BacWGSpipe automatically generates annotation files of MLST, AMR, and VF in a format supported by ITOL that can be customized by users. 
...
[1] Chen, Shifu et al. "fastp: an ultra-fast all-in-one FASTQ preprocessor." Bioinformatics (Oxford, England) vol. 34,17 (2018): i884-i890. doi:10.1093/bioinformatics/bty560

[2] Wood, Derrick E et al. "Improved metagenomic analysis with Kraken 2." Genome biology vol. 20,1 257. 28 Nov. 2019, doi:10.1186/s13059-019-1891-0

[3] Wick, Ryan R et al. "Unicycler: Resolving bacterial genome assemblies from short and long sequencing reads." PLoS computational biology vol. 13,6 e1005595. 8 Jun. 2017, doi:10.1371/journal.pcbi.1005595

[4] Seemann, Torsten. "Prokka: rapid prokaryotic genome annotation." Bioinformatics (Oxford, England) vol. 30,14 (2014): 2068-9. doi:10.1093/bioinformatics/btu153

[5] Carattoli, Alessandra et al. "In silico detection and typing of plasmids using PlasmidFinder and plasmid multilocus sequence typing." Antimicrobial agents and chemotherapy vol. 58,7 (2014): 3895-903. doi:10.1128/AAC.02412-14

[6] Schwengers, Oliver et al. "Platon: identification and characterization of bacterial plasmid contigs in short-read draft assemblies exploiting protein sequence-based replicon distribution scores." Microbial genomics vol. 6,10 (2020): mgen000398. doi:10.1099/mgen.0.000398

[7] Starikova, Elizaveta V et al. "Phigaro: high-throughput prophage sequence annotation." Bioinformatics (Oxford, England) vol. 36,12 (2020): 3882-3884. doi:10.1093/bioinformatics/btaa250

[8] Liu, Meng et al. "ICEberg 2.0: an updated database of bacterial integrative and conjugative elements." Nucleic acids research vol. 47,D1 (2019): D660-D665. doi:10.1093/nar/gky1123

[9] Puterová, Janka, and Tomáš Martínek. "digIS: towards detecting distant and putative novel insertion sequence elements in prokaryotic genomes." BMC bioinformatics vol. 22,1 258. 20 May. 2021, doi:10.1186/s12859-021-04177-6

[10] Bertelli, Claire, and Fiona S L Brinkman. "Improved genomic island predictions with IslandPath-DIMOB." Bioinformatics (Oxford, England) vol. 34,13 (2018): 2161-2167. doi:10.1093/bioinformatics/bty095

[11] Russel, Jakob et al. "CRISPRCasTyper: Automated Identification, Annotation, and Classification of CRISPR-Cas Loci." The CRISPR journal vol. 3,6 (2020): 462-469. doi:10.1089/crispr.2020.0059

[12] Cantalapiedra, Carlos P et al. "eggNOG-mapper v2: Functional Annotation, Orthology Assignments, and Domain Prediction at the Metagenomic Scale." Molecular biology and evolution vol. 38,12 (2021): 5825-5829. doi:10.1093/molbev/msab293

[13] Huerta-Cepas, Jaime et al. "eggNOG 5.0: a hierarchical, functionally and phylogenetically annotated orthology resource based on 5090 organisms and 2502 viruses." Nucleic acids research vol. 47,D1 (2019): D309-D314. doi:10.1093/nar/gky1085

[14] Ashburner, M et al. "Gene ontology: tool for the unification of biology. The Gene Ontology Consortium." Nature genetics vol. 25,1 (2000): 25-9. doi:10.1038/75556

[15] Kanehisa, M, and S Goto. "KEGG: kyoto encyclopedia of genes and genomes." Nucleic acids research vol. 28,1 (2000): 27-30. doi:10.1093/nar/28.1.27

[16] Tatusov, Roman L et al. "The COG database: an updated version includes eukaryotes." BMC bioinformatics vol. 4 (2003): 41. doi:10.1186/1471-2105-4-41

[17] Federhen, Scott. "The NCBI Taxonomy database." Nucleic acids research vol. 40,Database issue (2012): D136-43. doi:10.1093/nar/gkr1178

[18] Teufel, Felix et al. "SignalP 6.0 predicts all five types of signal peptides using protein language models." Nature biotechnology vol. 40,7 (2022): 1023-1025. doi:10.1038/s41587-021-01156-3

[19] Krogh, A et al. "Predicting transmembrane protein topology with a hidden Markov model: application to complete genomes." Journal of molecular biology vol. 305,3 (2001): 567-80. doi:10.1006/jmbi.2000.4315

[20] Chen, Lihong et al. "VFDB: a reference database for bacterial virulence factors." Nucleic acids research vol. 33,Database issue (2005): D325-8. doi:10.1093/nar/gki008

[21] Camacho, Christiam et al. "BLAST+: architecture and applications." BMC bioinformatics vol. 10 421. 15 Dec. 2009, doi:10.1186/1471-2105-10-421

[22] McArthur, Andrew G et al. "The comprehensive antibiotic resistance database." Antimicrobial agents and chemotherapy vol. 57,7 (2013): 3348-57. doi:10.1128/AAC.00419-13

[23] Alcock, Brian P et al. "CARD 2020: antibiotic resistome surveillance with the comprehensive antibiotic resistance database." Nucleic acids research vol. 48,D1 (2020): D517-D525. doi:10.1093/nar/gkz935

[24] Jiang, Jianping et al. "Carbapenemase-Encoding Gene Copy Number Estimator (CCNE): a Tool for Carbapenemase Gene Copy Number Estimation." Microbiology spectrum vol. 10,4 (2022): e0100022. doi:10.1128/spectrum.01000-22

[25] Seemann T, mlst Github https://github.com/tseemann/mlst

[26] Jolley, Keith A et al. "Open-access bacterial population genomics: BIGSdb software, the PubMLST.org website and their applications." Wellcome open research vol. 3 124. 24 Sep. 2018, doi:10.12688/wellcomeopenres.14826.1

[27] A. J. Page et al., "Roary: rapid large-scale prokaryote pan genome analysis," Bioinformatics, vol. 31, no. 22, pp. 3691–3693, 2015.

[28] L.-T. Nguyen, H. A. Schmidt, A. Von Haeseler, and B. Q. Minh, "IQ-TREE: a fast and effective stochastic algorithm for estimating maximum-likelihood phylogenies," Molecular biology and evolution, vol. 32, no. 1, pp. 268–274, 2015.

[29] I. Letunic and P. Bork, "Interactive Tree Of Life (iTOL) v5: an online tool for phylogenetic tree display and annotation," Nucleic acids research, vol. 49, no. W1, pp. W293–W296, 2021.
